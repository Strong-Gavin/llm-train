...
train_dataset = dataset.map(process_func,fn_kwargs={"tokenizer": tokenizer, "max_seq_length": 2048},remove_columns=dataset.column_names)
...
lora_config = LoraConfig(
        r=16,  # 设置Lora低秩矩阵的秩（维度），建议是8，16，32，64，128。值越大，模型越准备，但是需要的显存越大，过拟合的风险越大。值越低，越省显存，但是容易减低准确率
...
train_args = TrainingArguments(
    ...
    num_train_epochs=3,
    learning_rate=1e-4,  # 学习率 范围建议1e-4(0.0001) 到5e-5(0.00005)
